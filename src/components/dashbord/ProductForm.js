...
export const ProductForm = ({ setShowForm }) => {
  ...
  useEffect(() => {
    const getCategories = async () => {
      try {
        const response = await axios.get("http://localhost:4000/api/v1/shops");
  ...
  const addProduct = async (e) => {
    ...
    try {
      await axios.post(
        "http://https://sbg.onrender.com/api/v1/products",
        form_data,
        {
          headers: {
  ...
  return (
    <div className="flex justify-center items-center overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none">
      ...
      >
        <div className="bg-primary py-2 px-4 text-center text-base font-normal text-white">
        ...
        <div className="flex flex-nowrap -mx-3 mb-6">
          <div className="rounded-lg shadow-xl bg-gray-50 px-3 lg:w-1/2">
            <div className="m-4">
        ...
        <div class="flex p-2 space-x-4">
          ...
          <button class="px-4 py-2 text-white bg-primary rounded shadow-xl">
            Create
          </button>
        </div>
      </form>
